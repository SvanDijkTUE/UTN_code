...
queue = 20*ones(nu,1);
 for k = 1:N
     if k == floor(N/2)
         disturbance = 2*UTN.Nominal_inflow;
     end
     x{k+1} = lower_dynamics(x{k},u{k}, disturbance, k, UTN);
     objective = objective + (x{k+1}(sel_links)-r(sel_links))'*Q*(x{k+1}(sel_links)-r(sel_links)) + 0.0001*u{k}'*u{k};
     constraints = [constraints, 10 <= u{k}<= 120, x{k+1}(sel_links)>=slack_var{k}(sel_links), slack_var{k}(sel_links)>=-50]; 
 ...
 xsim = lower_dynamics(xsim,value(u{1}), disturbance, 1, UTN);
 Xhist(:,t+1) = xsim; 
 Uhist(:,t) = value(u{1}); 
